...
import { Plus, X, Download, Building2, ArrowLeft, Check, FileSpreadsheet, Upload } from 'lucide-react'
...
// Simple CSV/Excel export without external library
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <div className="relative">
            <button onClick={() => document.getElementById('tracker-export-menu').classList.toggle('hidden')} className="btn-ghost text-xs">
            ...
            <div id="tracker-export-menu" className="hidden absolute right-0 top-full mt-1 bg-navy-800 border border-navy-600 rounded-xl shadow-xl z-20 min-w-36 overflow-hidden">
              <button onClick={()=>{exportToJSON(applications,notes);document.getElementById('tracker-export-menu').classList.add('hidden')}} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-slate-300 hover:bg-navy-700">
                <Download size={12}/> JSON
              </button>
              <button onClick={()=>{exportToCSV(applications);document.getElementById('tracker-export-menu').classList.add('hidden')}} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-slate-300 hover:bg-navy-700">
      ...
      {showAdd && (
        <div className="card mb-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-3">
            <input className="input-field" placeholder="Company *" value={newApp.company} onChange={e=>setNewApp(p=>({...p,company:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&addApplication()} />
                    ...
                    {stageApps.map(app=>(
                      <div key={app.id} className="card p-3">
                        <div className="flex items-start justify-between gap-1 mb-1">
                          <button onClick={()=>setSelectedApp(app)} className="text-white text-xs font-body font-semibold hover:text-teal-400 text-left leading-snug">
                            {app.company}
                          </button>
                          <button onClick={()=>deleteApp(app.id)} className="text-slate-600 hover:text-red-400 flex-shrink-0 ml-1"><X size={12}/></button>
                        </div>
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto animate-in">
      ...
      <div className="space-y-3">
        {[
          ...
          ['prepNotes','My prep notes','Key points to emphasize...'],
        ].map(([key,label,ph])=>(
          <div key={key}>
            <label className="text-sm text-slate-400 mb-1.5 block">{label}</label>
            <textarea className="textarea-field h-16" placeholder={ph} value={form[key]||''} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} />
          </div>
        ))}
      </div>
